...
class DioUtils {
  ...
  DioUtils._internal() {
    ...
    //当app运行再release环境时，inProduction位true
...
//发起请求
    try {
      _dio.options = await buildOption(_dio.options);
      _dio.options.headers["content-type"] = "application/json";
      Response response = await _dio.get(
          url, queryParameters: queryParameters, cancelToken: cancelTag);
...
//获取数据
  ...
  Future<ResponseInfo> postRequest(
      {required String url, Map<String, dynamic>? formDataMap, Map<
          String,
          dynamic>? jsonMap, CancelToken, cancelTag}) async {
    FormData? form;
    if (formDataMap != null) {
...
//发起post请求
    try {
      Response response = await _dio.post(
          url, data: form == null ? jsonMap : form, cancelToken: cancelTag);
//响应数据
      dynamic responseData = response.data;
      if (responseData is Map<String, dynamic>) {
        Map<String, dynamic> responseMap = responseData;
...
//业务异常处理
  ...
  Future<BaseOptions> buildOption(BaseOptions options) async {
    ///请求header配置
